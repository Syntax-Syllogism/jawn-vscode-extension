...
suite('Jawn extension', () => {
	...
	test('gatherInputs emits output-path only when a folder is picked', async () => {
		const command = commandById('jawn.aep.generate.selector');
		const store = new LastValueStore(new MemoryMemento());
		const inputApi: InputApi = {
			pickOrg: async () => 'dev',
			pickFile: async () => undefined,
			pickFolder: async () => 'force-app',
			showInputBox: async (options) => options.prompt?.includes('SObject') ? 'Account' : '',
		...
		const cancelFolderApi: InputApi = {
			...inputApi,
			pickFolder: async () => undefined,
		};
		const cancelledFolder = await gatherInputs(command, new LastValueStore(new MemoryMemento()), cancelFolderApi);
		assert.ok(!cancelledFolder?.args.includes('--output-path'));
	});
